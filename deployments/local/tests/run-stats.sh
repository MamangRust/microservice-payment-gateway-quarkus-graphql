...
STATS_MERCHANT="Stats Merchant ${STATS_EMAIL}"
...
cleanup() {
  local id
  id="$(docker exec postgres psql -U DRAGON -d PAYMENT_GATEWAY -Atc "select id from users where email='$STATS_EMAIL';" 2>/dev/null || true)"
  [[ -z "$id" ]] || docker exec postgres psql -U DRAGON -d PAYMENT_GATEWAY -c "delete from users where id=$id;" >/dev/null 2>&1 || true
}
trap cleanup EXIT

wait_for_gateway
BODY=/tmp/payment-gateway-stats-register.json
STATUS="$(curl --silent --show-error --max-time 15 -o "$BODY" -w '%{http_code}' \
  -H 'Content-Type: application/json' \
  -d "{\"firstname\":\"Stats\",\"lastname\":\"Runner\",\"email\":\"$STATS_EMAIL\",\"password\":\"E2E-password-123\",\"confirmPassword\":\"E2E-password-123\"}" \
  "$BASE_URL/api/auth/register")"
[[ "$STATUS" == 201 ]] || { cat "$BODY" >&2; exit 1; }
USER_ID="$(docker exec postgres psql -U DRAGON -d PAYMENT_GATEWAY -Atc "select id from users where email='$STATS_EMAIL';")"
...
hurl --test \
  ...
  "$ROOT_DIR/deployments/local/tests/stats.hurl"
