...
trap cleanup EXIT
wait_for_gateway
BODY=/tmp/payment-gateway-fraud-register.json
STATUS="$(curl --silent --show-error --max-time 15 -o "$BODY" -w '%{http_code}' -H 'Content-Type: application/json' -d "{\"firstname\":\"Fraud\",\"lastname\":\"Runner\",\"email\":\"$E2E_EMAIL\",\"password\":\"E2E-password-123\",\"confirmPassword\":\"E2E-password-123\"}" "$BASE_URL/api/auth/register")"
...
hurl --test --variable base_url="$BASE_URL" --variable e2e_email="$E2E_EMAIL" --variable user_id="$USER_ID" "$ROOT_DIR/deployments/local/tests/fraud-scoring.hurl"
echo "Financial risk smoke passed for $E2E_EMAIL"
